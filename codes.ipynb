...
def getuserlist(n):
    friendslist = [XXXXXXXX]  #The first ID，replace with your own random uid
    linklist = ['https://steamcommunity.com/profiles/XXXXXXXXXX/recommended/']#replace with your own random corresponding url
...
##
def remove(text): #Use Regular Expression to remove symbols
    remove_chars = '[0-9’!"#$%&\'()*+,-./:;<=>?@，。?★、…【】《》？“”‘’！[\\]^_`{|}~]+'
...
def getvalue(soup):
    ...
    for i in a:
        ...
        if b is not None:
            ...
            value.append('error')
    return value
def getfun(soup): #Count the number of people who found this review funny
    a = soup.find_all(class_="header")
    fun = []
    for i in a:
        ...
        if b is not None:
            ...
            fun.append('error')
    return fun             
def getreviews(soup):  #Get the content of reviews
    reviews = []
...
#         reviews.append(b.group(1))
    ...
    return reviews   
def getgame(soup): #Capture the game title
    games = []
    a = soup.find_all(class_="game_capsule_ctn")
    for i in a:
        games.append(i['href'])
    return games      
def getdate(soup):   #Get the date that the review was posted
    date = []
    ...
    return date  
def getplaytime(soup):  #Get the time that the reviewer have played on this game till now and the time when he wrote the review 
    total = []
    whenreview = []
